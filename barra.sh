...
load() {
    ...
    echo -ne "^p(_RIGHT)^p(-310)^fg($blue)⭦ ^fg($fg)$mem^fg($yellow)  ⭥ ^fg($fg)$cpu"
}

clock() {
    timea=$(date +'%b %d %a %H:%M')
    echo "^p(_RIGHT)^p(-113)^fg($magenta)⭧ ^fg($fg)$timea\r"
...
music() {
    ...
    echo -ne "^p(_LEFT)^fg($muscolor)⭯ ^fg($fg)$songa ^fg($fg)$songb\r"
}
